#!/usr/bin/env python3
"""
Test Refined Platelets (Platelet Count) ECL Query
==================================================
Platelets: Thrombocyte count in blood

Component: Fixed 16378004 |Platelet| (no descendants)
...
OUTPUT_DIR = PROJECT_ROOT / 'output' / 'singular_concepts' / 'blood_count_platelets'
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("REFINED PLATELETS (PLATELET COUNT) ECL TEST")
print("=" * 80)
...
# Define ECL queries for platelets
# Component: 16378004 |Platelet| (fixed, no descendants)
# Property: 118550005 |Number concentration|
# Exclude plasma specimens (119361006) - not relevant for routine CBC
ecl_queries = {
    'platelets_fixed_component_excl_cord_plasma': """
<< 363787002 |Observable entity| :
    << 246093002 |Component| = 16378004 |Platelet|,
    ...
    << 704327008 |Direct site| != << 119361006 |Plasma specimen|
""".strip(),
    'platelets_fixed_component_incl_cord_excl_plasma': """
<< 363787002 |Observable entity| :
    << 246093002 |Component| = 16378004 |Platelet|,
    << 370130000 |Property| = << 118550005 |Number concentration|,
    << 704327008 |Direct site| = << 119297000 |Blood specimen|,
...
# Execute queries
print("[3/4] Executing platelet ECL queries...")
results = {}
...
# Save results
output_file = OUTPUT_DIR / 'platelets_ecl_test_results.json'
...
for loinc_code in sorted(all_loinc_codes):
    ...
    in_fixed_excl = 'Yes' if loinc_code in results['platelets_fixed_component_excl_cord_plasma']['loinc_codes'] else ''
    in_fixed_incl = 'Yes' if loinc_code in results['platelets_fixed_component_incl_cord_excl_plasma']['loinc_codes'] else ''

    # Get SNOMED IDs
    snomed_ids = []
    if loinc_code in results['platelets_fixed_component_excl_cord_plasma']['snomed_to_loinc_mapping']:
        snomed_ids = [m['snomed_id'] for m in results['platelets_fixed_component_excl_cord_plasma']['snomed_to_loinc_mapping'][loinc_code]]
    elif loinc_code in results['platelets_fixed_component_incl_cord_excl_plasma']['snomed_to_loinc_mapping']:
        snomed_ids = [m['snomed_id'] for m in results['platelets_fixed_component_incl_cord_excl_plasma']['snomed_to_loinc_mapping'][loinc_code]]

    rows.append({
        'LOINC_Code': loinc_code,
        'LOINC_Display': loinc_displays.get(loinc_code, ''),
        'FixedComp_Excl_Cord_Plasma': in_fixed_excl,
        'FixedComp_Incl_Cord_Excl_Plasma': in_fixed_incl,
        'SNOMED_IDs': '; '.join(snomed_ids) if snomed_ids else ''
    })

df_out = pd.DataFrame(rows)
csv_file = OUTPUT_DIR / 'platelets_ecl_comparison.csv'
