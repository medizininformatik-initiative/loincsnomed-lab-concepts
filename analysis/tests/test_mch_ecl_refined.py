#!/usr/bin/env python3
"""
Test Refined MCH (Mean Corpuscular Hemoglobin) ECL Query
==========================================================
Primary LOINC: 28539-5

MCH is a calculated parameter representing the average mass of hemoglobin
per red blood cell. It's a key erythrocyte index used in anemia classification.
"""
...
print("=" * 80)
print("REFINED MCH (MEAN CORPUSCULAR HEMOGLOBIN) ECL TEST")
print("=" * 80)
...
# Define ECL queries for MCH
# Using pre-coordinated parent concept: 521361010000106 |Entitic mass of hemoglobin in erythrocyte|
# This parent concept includes both method-specific and non-specific LOINC codes
ecl_queries = {
    'mch_precoord_descendants': """
<< 521361010000106 |Entitic mass of hemoglobin in erythrocyte at point in time|
""".strip()
}

# Execute queries
print("[3/4] Executing MCH ECL queries...")
results = {}
...
# Save results
output_file = OUTPUT_DIR / 'mch_ecl_test_results.json'
...
for loinc_code in sorted(all_loinc_codes):
    ...
    in_precoord = 'Yes' if loinc_code in results['mch_precoord_descendants']['loinc_codes'] else ''

    # Get SNOMED IDs
    snomed_ids = []
    if loinc_code in results['mch_precoord_descendants']['snomed_to_loinc_mapping']:
        snomed_ids = [m['snomed_id'] for m in results['mch_precoord_descendants']['snomed_to_loinc_mapping'][loinc_code]]
...
csv_file = OUTPUT_DIR / 'mch_ecl_comparison.csv'
