#!/usr/bin/env python3
"""
Test Refined MCHC (Mean Corpuscular Hemoglobin Concentration) ECL Query
========================================================================
MCHC: Average concentration of hemoglobin per erythrocyte

Component: Could be Hemoglobin or Erythrocyte - needs investigation
Property: Mass concentration or Substance concentration
"""
...
print("=" * 80)
print("REFINED MCHC (MEAN CORPUSCULAR HEMOGLOBIN CONCENTRATION) ECL TEST")
print("=" * 80)
...
# Define ECL queries for MCHC
# Using pre-coordinated parent concept: 521371010000101 |Entitic mass concentration of hemoglobin in erythrocyte|
# This parent concept includes both method-specific and non-specific LOINC codes
ecl_queries = {
    'mchc_precoord_descendants': """
<< 521371010000101 |Entitic mass concentration of hemoglobin in erythrocyte at point in time|
""".strip()
}

# Execute queries
print("[3/4] Executing MCHC ECL queries...")
results = {}
...
# Save results
output_file = OUTPUT_DIR / 'mchc_ecl_test_results.json'
...
for loinc_code in sorted(all_loinc_codes):
    ...
    in_precoord = 'Yes' if loinc_code in results['mchc_precoord_descendants']['loinc_codes'] else ''

    # Get SNOMED IDs
    snomed_ids = []
    if loinc_code in results['mchc_precoord_descendants']['snomed_to_loinc_mapping']:
        snomed_ids = [m['snomed_id'] for m in results['mchc_precoord_descendants']['snomed_to_loinc_mapping'][loinc_code]]
...
csv_file = OUTPUT_DIR / 'mchc_ecl_comparison.csv'
