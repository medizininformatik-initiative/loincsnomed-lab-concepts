#!/usr/bin/env python3
"""
Test Refined Hematocrit ECL Query
==================================
Hematocrit: Volume fraction of erythrocytes in blood

Key difference from RBC count:
- Component: Same (41898006 |Erythrocyte|)
- Property: 118586006 |Volume fraction| (not 118550005 |Number concentration|)
- This gives hematocrit (volume fraction) instead of cell count
"""
...
OUTPUT_DIR = PROJECT_ROOT / 'output' / 'singular_concepts' / 'blood_count_hematocrit'
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("REFINED HEMATOCRIT ECL TEST")
print("=" * 80)
...
# Define ECL queries for hematocrit
# Component: 41898006 |Erythrocyte| (fixed, no descendants)
# Property: 118586006 |Volume fraction|
# This captures hematocrit, not RBC count (which uses 118550005 |Number concentration|)
ecl_queries = {
    'hematocrit_fixed_component_excl_cord': """
<< 363787002 |Observable entity| :
    << 246093002 |Component| = 41898006 |Erythrocyte|,
    << 370130000 |Property| = << 118586006 |Volume fraction|,
    << 704327008 |Direct site| = << 119297000 |Blood specimen|,
    << 704327008 |Direct site| != << 122556008 |Cord blood specimen|
""".strip(),
    'hematocrit_fixed_component_incl_cord': """
<< 363787002 |Observable entity| :
    << 246093002 |Component| = 41898006 |Erythrocyte|,
    << 370130000 |Property| = << 118586006 |Volume fraction|,
    << 704327008 |Direct site| = << 119297000 |Blood specimen|
...
# Execute queries
print("[3/4] Executing hematocrit ECL queries...")
results = {}
...
# Save results
output_file = OUTPUT_DIR / 'hematocrit_ecl_test_results.json'
...
for loinc_code in sorted(all_loinc_codes):
    ...
    in_fixed_excl = 'Yes' if loinc_code in results['hematocrit_fixed_component_excl_cord']['loinc_codes'] else ''
    in_fixed_incl = 'Yes' if loinc_code in results['hematocrit_fixed_component_incl_cord']['loinc_codes'] else ''

    # Get SNOMED IDs
    snomed_ids = []
    if loinc_code in results['hematocrit_fixed_component_excl_cord']['snomed_to_loinc_mapping']:
        snomed_ids = [m['snomed_id'] for m in results['hematocrit_fixed_component_excl_cord']['snomed_to_loinc_mapping'][loinc_code]]
    elif loinc_code in results['hematocrit_fixed_component_incl_cord']['snomed_to_loinc_mapping']:
        snomed_ids = [m['snomed_id'] for m in results['hematocrit_fixed_component_incl_cord']['snomed_to_loinc_mapping'][loinc_code]]
...
csv_file = OUTPUT_DIR / 'hematocrit_ecl_comparison.csv'
