#!/usr/bin/env python3
r"""
ECL Fixed Component + System Experiment - Run All Steps
========================================================
Component + System (Direct site) constrained ECL queries.
Extracts Component and Direct site attributes from SNOMED concepts and builds:
<< 363787002 |Observable entity| :
   246093002 |Component| = [Component_ID],
   704327008 |Direct site| = [System_ID]
...
OUTPUT_DIR = PROJECT_ROOT / 'output' / 'ecl_fixed_component_system'
...
DIRECT_SITE_ID = "704327008"          # Direct site (System/specimen)

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
(OUTPUT_DIR / 'valuesets').mkdir(exist_ok=True)

print("="*80)
print("ECL FIXED COMPONENT + SYSTEM EXPERIMENT")
print("="*80)
print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

# ==============================================================================
# Helper: Load Component and System Relationships from File
# ==============================================================================
def load_component_and_system_relationships(relationship_file):
    """
    Load Component and Direct site (System) relationships from SNOMED Relationship file.

    Returns:
        dict mapping concept_id -> {'component_id': str, 'system_id': str}
    """
            ...
            if len(parts) >= 8:
                ...
                if active == '1':
                    ...
                    if type_id == COMPONENT_ATTRIBUTE_ID:
                        ...
                        concept_map[source_id]['system_id'] = destination_id
...
# ==============================================================================
# STEP 3: Extract Component and System Attributes from SNOMED
# ==============================================================================
print("\n[STEP 3/5] Extracting Component and System attributes from SNOMED concepts...")

# Load all component and system relationships
concept_map = load_component_and_system_relationships(RELATIONSHIP_FILE)
...
for primary_loinc, snomed_id in primary_to_snomed.items():
    if snomed_id in concept_map:
        ...
        if component_id and system_id:
            primary_to_attributes[primary_loinc] = {
                'component_id': component_id,
                'system_id': system_id
            }
            print(f"  {primary_loinc} -> Component: {component_id}, System: {system_id}")
        elif component_id:
            print(f"  {primary_loinc} -> Component: {component_id}, System: MISSING")
        elif system_id:
            print(f"  {primary_loinc} -> Component: MISSING, System: {system_id}")
        else:
            print(f"  {primary_loinc} -> No Component or System found")
    else:
        print(f"  {primary_loinc} -> No attributes found")

print(f"\n  [OK] Extracted Component+System for {len(primary_to_attributes)}/{len(primary_to_snomed)} primary codes")
...
mapping_output = {
    ...
    'summary': {
        ...
        'primary_with_component_and_system': len(primary_to_attributes),
    ...
    'mappings': {
        primary: {
            ...
            'system_id': primary_to_attributes.get(primary, {}).get('system_id'),
...
# ==============================================================================
# STEP 4: Execute ECL Queries (Component + System Based)
# ==============================================================================
print("\n[STEP 4/5] Executing Component + System ECL queries...")
print("  ECL Strategy: << 363787002 |Observable entity| :")
print("                   246093002 |Component| = [Component_ID],")
print("                   704327008 |Direct site| = [System_ID]")
...
for primary_loinc, attrs in primary_to_attributes.items():
    ...
    print(f"      Component: {component_id}, System: {system_id}")

    # Build Component + System ECL (both fixed, no descendants)
    ecl = f"<< 363787002 |Observable entity| : 246093002 |Component| = {component_id}, 704327008 |Direct site| = {system_id}"
    print(f"      ECL: {ecl}")
    ...
    valueset = {
        ...
        "id": f"ecl-component-system-{primary_loinc.replace('-', '')}",
        "url": f"https://www.medizininformatik-initiative.de/fhir/ext/modul-labor/ValueSet/ecl-component-system-{primary_loinc.replace('-', '')}",
        "version": "1.0",
        "name": f"ECLComponentSystem{primary_loinc.replace('-', '')}",
        "title": f"ECL Component+System: {primary_to_name.get(primary_loinc, primary_loinc)}",
        "status": "draft",
        "experimental": True,
        "date": datetime.now().strftime("%Y-%m-%d"),
        "publisher": "MII INTERPOLAR - ECL Experiment",
        "description": f"ECL-generated ValueSet using Component={component_id} AND System={system_id} for LOINC {primary_loinc}",
        "compose": {
            "include": [
                {
    ...
    vs_file = OUTPUT_DIR / 'valuesets' / f"valueset-ecl-component-system-{primary_loinc.replace('-', '')}.json"
