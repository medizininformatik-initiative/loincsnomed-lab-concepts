#!/usr/bin/env python3
r"""
ECL Fixed Component + Property Experiment - Run All Steps
========================================================
Component + Property (Property) constrained ECL queries.
Extracts Component and Property attributes from SNOMED concepts and builds:
<< 363787002 |Observable entity| :
   246093002 |Component| = [Component_ID],
   370130000 |Property| = [Property_ID]

Expected: Higher precision by constraining to specific specimen type.
...
OUTPUT_DIR = PROJECT_ROOT / 'output' / 'ecl_fixed_component_property'
...
DIRECT_SITE_ID = "370130000"          # Property (Property/specimen)

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
(OUTPUT_DIR / 'valuesets').mkdir(exist_ok=True)

print("="*80)
print("ECL FIXED COMPONENT + PROPERTY EXPERIMENT")
print("="*80)
print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

# ==============================================================================
# Helper: Load Component and Property Relationships from File
# ==============================================================================
def load_component_and_property_relationships(relationship_file):
    """
    Load Component and Property (Property) relationships from SNOMED Relationship file.

    Returns:
        dict mapping concept_id -> {'component_id': str, 'property_id': str}
    """
    ...
    with open(relationship_file, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if len(parts) >= 8:
                ...
                if active == '1':
                    if source_id not in concept_map:
                        concept_map[source_id] = {}

                    if type_id == COMPONENT_ATTRIBUTE_ID:
                        concept_map[source_id]['component_id'] = destination_id
                    elif type_id == DIRECT_SITE_ID:
                        concept_map[source_id]['property_id'] = destination_id

    print(f"  [OK] Loaded relationships for {len(concept_map)} concepts")
    return concept_map
...
# ==============================================================================
# STEP 3: Extract Component and Property Attributes from SNOMED
# ==============================================================================
print("\n[STEP 3/5] Extracting Component and Property attributes from SNOMED concepts...")

# Load all component and property relationships
concept_map = load_component_and_property_relationships(RELATIONSHIP_FILE)

primary_to_attributes = {}

for primary_loinc, snomed_id in primary_to_snomed.items():
    if snomed_id in concept_map:
        attrs = concept_map[snomed_id]
        component_id = attrs.get('component_id')
        property_id = attrs.get('property_id')

        if component_id and property_id:
            primary_to_attributes[primary_loinc] = {
                'component_id': component_id,
                'property_id': property_id
            }
            print(f"  {primary_loinc} -> Component: {component_id}, Property: {property_id}")
        elif component_id:
            print(f"  {primary_loinc} -> Component: {component_id}, Property: MISSING")
        elif property_id:
            print(f"  {primary_loinc} -> Component: MISSING, Property: {property_id}")
        else:
            print(f"  {primary_loinc} -> No Component or Property found")
    else:
        print(f"  {primary_loinc} -> No attributes found")

print(f"\n  [OK] Extracted Component+Property for {len(primary_to_attributes)}/{len(primary_to_snomed)} primary codes")
...
mapping_output = {
    ...
    'summary': {
        ...
        'primary_with_component_and_property': len(primary_to_attributes),
    ...
    'mappings': {
        primary: {
            ...
            'component_id': primary_to_attributes.get(primary, {}).get('component_id'),
            'property_id': primary_to_attributes.get(primary, {}).get('property_id'),
...
# ==============================================================================
# STEP 4: Execute ECL Queries (Component + Property Based)
# ==============================================================================
print("\n[STEP 4/5] Executing Component + Property ECL queries...")
print("  ECL Strategy: << 363787002 |Observable entity| :")
print("                   246093002 |Component| = [Component_ID],")
print("                   370130000 |Property| = [Property_ID]")
...
for primary_loinc, attrs in primary_to_attributes.items():
    processed += 1
    component_id = attrs['component_id']
    property_id = attrs['property_id']

    print(f"\n  [{processed}/{len(primary_to_attributes)}] Processing {primary_loinc}")
    print(f"      Name: {primary_to_name.get(primary_loinc, 'N/A')}")
    print(f"      Component: {component_id}, Property: {property_id}")

    # Build Component + Property ECL (both fixed, no descendants)
    ecl = f"<< 363787002 |Observable entity| : 246093002 |Component| = {component_id}, 370130000 |Property| = {property_id}"
    print(f"      ECL: {ecl}")
    ...
    valueset = {
        ...
        "id": f"ecl-component-property-{primary_loinc.replace('-', '')}",
        "url": f"https://www.medizininformatik-initiative.de/fhir/ext/modul-labor/ValueSet/ecl-component-property-{primary_loinc.replace('-', '')}",
        "version": "1.0",
        "name": f"ECLComponentProperty{primary_loinc.replace('-', '')}",
        "title": f"ECL Component+Property: {primary_to_name.get(primary_loinc, primary_loinc)}",
        "status": "draft",
        "experimental": True,
        "date": datetime.now().strftime("%Y-%m-%d"),
        "publisher": "MII INTERPOLAR - ECL Experiment",
        "description": f"ECL-generated ValueSet using Component={component_id} AND Property={property_id} for LOINC {primary_loinc}",
        "compose": {
            "include": [
                {
    ...
    vs_file = OUTPUT_DIR / 'valuesets' / f"valueset-ecl-component-property-{primary_loinc.replace('-', '')}.json"
...
for primary_loinc in primary_to_attributes.keys():
